...
if (!(isset($teacherid))) {
	...
	if (isset($_GET['action'])) {
		...
		if (mysqli_num_rows($result)==0) {
			...
			$result = mysqli_query($GLOBALS['link'],$query) or die("Query failed : " . mysqli_error($GLOBALS['link']));
			list($ekey, $copyrights) = mysqli_fetch_row($result);
			$copyrights *= 1;
			if ($copyrights<2) {
				$oktocopy = 0;
				if ($copyrights==1) {
	...
	if (isset($_GET['action']) && $_GET['action']=="copycalitems") {
		...
		if (isset($_POST['checked'])) {
			...
			$result = mysqli_query($GLOBALS['link'],$query) or die("Query failed : $query" . mysqli_error($GLOBALS['link']));
			$blockcnt = mysqli_fetch_first($result);
		...
		if (isset($_POST['copyoffline'])) {
			...
			while ($row = mysqli_fetch_row($result)) {
				...
				mysqli_query($GLOBALS['link'],$query) or die("Query failed :$query " . mysqli_error($GLOBALS['link']));
				if ($rubric>0) {
					$offlinerubrics[mysqli_insert_id($GLOBALS['link'])] = $rubric;
				}
			}
		}
		...
		$courseGroupResults = mysqli_query($GLOBALS['link'],$query) or die("Query failed : $query: " . mysqli_error($GLOBALS['link']));
		...
		$courseTreeResult = mysqli_query($GLOBALS['link'],$query) or die("Query failed : " . mysqli_error($GLOBALS['link']));
		$lastteacher = 0;
		...
		$courseTemplateResults = mysqli_query($GLOBALS['link'],$query) or die("Query failed : " . mysqli_error($GLOBALS['link']));
	}
}
/******* begin html output ********/
...
<?php
	 ...
	 if ($page_hasGroups) {
				$lastteacher = 0;
				$lastgroup = -1;
				while ($line = mysqli_fetch_assoc($courseGroupResults)) {
...
<?php
//group's courses
		if (mysqli_num_rows($courseTreeResult)>0) {
			while ($line = mysqli_fetch_assoc($courseTreeResult)) {
				if ($line['userid']!=$lastteacher) {
					if ($lastteacher!=0) {
						echo "				</ul>\n			</li>\n";
					}
...
<?php			
			while ($row = mysqli_fetch_row($courseTemplateResults)) {
?>			
				<li>
					<span class=dd>-</span>
